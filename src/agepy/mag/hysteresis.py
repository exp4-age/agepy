...
def normalize(*magnetization: Sequence, 
    ...
    *magnetization: Sequence
    ...
    mean_range = np.ceil(len(branch_1) * saturation_mean_percentage / 100).astype(int)

    # normalize
    max_mag = np.mean(np.concatenate((branch_1[:mean_range], branch_2[len(branch_2) - mean_range:])))
    min_mag = np.mean(np.concatenate((branch_2[:mean_range], branch_1[len(branch_2) - mean_range:])))
...
def faraday_correction(*hysteresis: ArrayLike, 
    ...
    *hysteresis: ArrayLike
    ...
    def functional(x):
        ...
        corrected = linear_correction((H, M - cos_squared), 
    ...
    logger.log(log_level, f"Computed minimum for faraday correction:\n{minimum}")
...
def optimized_faraday_correction(*hysteresis: ArrayLike, 
    ...
    *hysteresis: ArrayLike
    ...
    def functional(scale):
        ...
        corrected = faraday_correction((H, M), initial_guess=(scale, 
                                                              verdet_guess, 
                                                              left_shift_guess),
                                        saturation_percentage=saturation_percentage,
    ...
    M_corrected = faraday_correction((H, M),
                                     initial_guess=(minimum.x,
                                                    verdet_guess,
                                                    left_shift_guess),
                                     saturation_percentage=saturation_percentage,
                                     log_level=log_level)
