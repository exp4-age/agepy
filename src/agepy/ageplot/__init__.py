...
class ageunit():
    ...
    def _parse_unit(self, unit):
        ...
        for u in units:
            if u in self.units:
                unit += self.units[u]  + r"\,"
            elif u in self.prefix:
                unit += self.prefix[u]
            else:
                raise ValueError(u + " is not a valid unit.")
    ...
    @staticmethod
    def nm(var_name):
        return var_name + r"$\;/\;$nm"

    @staticmethod
    def angstrom(var_name):
        return var_name + r"$\;/\;$Å"

    @staticmethod
    def angstrom(var_name):
        return var_name + r"$\;/\;$eV"

    @staticmethod
    def arbu(var_name):
        return var_name + r"$\;/\;$arb.$\,$u.$"

    @staticmethod
    def au(var_name):
        return var_name + r"$\;/\;$a.$\,$u.$"